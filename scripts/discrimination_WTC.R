...
####read all tdl files and run tdl formating and xsi functions on each list element-------------------------------------
  ...
  oct_gm <- lapply(oct_H4, gmcalc_func2)
  ...
  dec_gm_h4 <- lapply(dec_h4, gmcalc_func2)
  dec_gm_h2 <- lapply(dec_h2, gmcalc_func2)
  ...
  jan_gm_h2 <- lapply(jan_h2, gmcalc_func2)
  jan_gm_h3 <- lapply(jan_h3, gmcalc_func2)
  ...
  feb_gm_r1 <- lapply(feb_r1, gmcalc_func2)
  feb_gm_h4 <- lapply(feb_h4, gmcalc_func2)
...
####march
  ...
  mar_gm_h3 <- lapply(mar_h3, gmcalc_func2)
  mar_gm_h2 <- lapply(mar_h2, gmcalc_func2)
  mar_gm_h1 <- lapply(mar_h1, gmcalc_func2)
  mar_gm_h4 <- lapply(mar_h4, gmcalc_func2)
  ...
  apr_gm_h1 <- lapply(apr_h1, gmcalc_func2)
  apr_gm_h3 <- lapply(apr_h3, gmcalc_func2)
  apr_gm_h4 <- lapply(apr_h4, gmcalc_func2)
...
write.csv(gm_wtc_pair, "calculated_data/discrim_wtc.csv", row.names=FALSE)

###for analysis first subset well watered--------------------------------------------------------


gm_water <- gm_WTC2[gm_WTC2$drydown == "control",]
write.csv(gm_water, "calculated_data/discrim_wellwatered.csv", row.names=FALSE)
