...
# leafk2 <- subset(transp_agg, transp_agg$leaflight !=  "shade-low")
# 
...
#   
#   # leafk2$Month <- as.factor(leafk2$Month)
#   # leafk2$chamber<- as.factor(leafk2$chamber)
#   # leafk2$leaf<- as.factor(leafk2$leaf)
#   #remade new dataset with K at shade high light
# 
# #merge
...
# #write.csv(leafK_nodrought, "calculated_data/leafK_nodrought_highlight.csv", row.names=FALSE)
# 
# 
# #plotting--------------------------------------------------------------------------------------
# ypos <- c(2.5,1,0)
...
# 
# 
# boxplot(pre_mp~temp+leaf, data=leafK_stats)
# title("predawn")
# boxplot(mid_mp~temp+leaf, data=leafK_stats)
# title("midday")
# 
# 
