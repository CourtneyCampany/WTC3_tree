...
wet_pair <- merge(wet_high_agg, wet_low_agg, by=c("chamber","Month"), all=TRUE)
  ...
  wet_pair2 <- arrange(wet_pair, gm.low)
    wet_pair3 <- addtrt_func(wet_pair2)

dry_pair <- merge(dry_high_agg, dry_low_agg, by=c("chamber","Month"), all=TRUE)
  dry_pair2 <- dry_pair[complete.cases(dry_pair),]
  dry_pair2 <- arrange(dry_pair, gm.low)
...
###Well watered
windows(10,8)
plot(Photo.high~gm.high, data=wet_pair2,  pch=16, col=lightscol, ylim=c(0,30), xlim=c(0,.3), 
     xlab=gmlab, ylab="", cex=1.5)
  points(Photo.low~gm.low, data=wet_pair2,pch=16, col=shacol, cex=1.5)
