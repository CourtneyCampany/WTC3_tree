...
aciparam <- read.csv("calculated_data/aciparameters_gm.csv")
...
plot(Vcmax~leafN_area.mean, data=N_aci[N_aci$leaf=="shade",], col=shacol,  ylim=c(40, 133),xlim=c(1,3.5),
       pch=c(16, 17)[pch=N_aci$temp],xlab=narealab, ylab=vclab)
  points(Vcmax~leafN_area.mean, data=N_aci[N_aci$leaf=="sun",], col=suncol, pch=c(16, 17)[pch=N_aci$temp])
...
# dev.copy2pdf(file="master_scripts/paper_figures/acc_nitrovcmax.pdf")
# dev.off()
