...
Ci_bar <- read.csv("calculated_data/Ci_bar_drought.csv")

gmes_dry <- read.csv("calculated_data/gmes_drought.csv")
##calculate CC
gmes_dry$Cc<- with(gmes_dry, Ci-Photo/gm)
gmes_dry$cc_ci<- with(gmes_dry, Ci/Cc)

###get average by id
gm_agg <- summaryBy(Photo+Cond+Ci+Trmmol+Cc+gm+VpdL+xsi+DELTA+cc_ci ~ chamber+id+leaf +light+temp+leaflight, 
                    data=gmes_dry, FUN=mean, keep.names=TRUE)
...
#SUN leaves
sunmod <- gam(Photo ~ s(Cond, k=3), data=gm_c13, subset=leaflight=="sun-high")
...
#SHADE leaves
shamod <- gam(Photo ~ s(Cond, k=3), data=gm_c13, subset=leaflight=="shade-low")
...
plot(Photo~Cond, data=gm_c13, subset=leaflight=="sun-high", pch=16, col=suncol, ylim=c(0,25), 
     xlim=c(0,.5), xlab=condlab, ylab=satlab, cex=1.25)
