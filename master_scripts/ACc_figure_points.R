...
gm_agg <-summaryBy(gm_bar ~ leaflight+temp, data=gm, FUN=mean, keep.names=TRUE)
...
acishade_clean <- acishade[acishade$chamber != "ch02",]
  ...
  acishade_clean$gm_bar <- ifelse(acishade_clean$temp=="ambient", gm_agg[3,3],gm_agg[4,3])
  acishade_clean$Cc <- with(acishade_clean, Ci-Photo/gm_bar)

  
shade_redo <- read.csv("raw data/shadeaci_redo.csv")
  shade_redo <- merge(shade_redo, treatments)
  ##calculate Cc using appropriate gmes
  shade_redo$gm_bar <- ifelse(shade_redo$temp=="ambient", gm_agg[3,3],gm_agg[4,3])
...
sunaci_clean <- sunaci[sunaci$chamber != "ch06", ]
  ...
  ##calculate Cc using appropriate gmes
  sunaci_clean2$gm_bar <- ifelse(sunaci_clean2$temp=="ambient", gm_agg[5,3],gm_agg[6,3])
  sunaci_clean2$Cc <- with(sunaci_clean2, Ci-Photo/gm_bar)
...
###Add points from ACi curves, where Cc is predicted
  ...
  points(Photo~Cc ,data= sunaci_clean2,  col=suncol50,  pch=c(16, 17)[pch=sunaci_clean2$temp])
